WITH order_item_measures AS (
	SELECT
		order_id,
		SUM(item_sales_price) AS total_sale_price,
		...
		SUM(IF(product_department = '{{department_name}}', item_sales_price, 0)) AS total_sold_{{department_name.lower()}}swear{% if not loop.last %},{% endif -%}
		{% endfor %}
...
SELECT
	...
	TIMESTAMP_DIFF(od.created_at, ud.first_order_created_at, DAY) AS days_since_first_order
FROM {{ ref('stg_ecommerce__orders') }} as od
	LEFT JOIN order_item_measures AS om
		ON od.order_id = om.order_id
	LEFT JOIN {{ ref('int_ecommerce__first_order_created') }} as ud
